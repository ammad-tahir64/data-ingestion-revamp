...
{
    public class E4EAIQueue : IE4EAIQueue
    {
        public async Task<bool> SendAsync(DeviceProcessing deviceProcessing, ILogger log)
        {

            ServiceBusClient serviceBusClient = new ServiceBusClient(Environment.GetEnvironmentVariable(ApplicationSettings.E4scoreEaiQueue));
            var sender = serviceBusClient.CreateSender("e4score-eai-queue");
